...
export const MindMapBlock: React.FC<MindMapBlockProps> = ({ root, tasks = [], onLinkToTask, onUnlinkTask, onChange, autoExpand }) => {
  ...
  const handleCanvasPointerDown = (e: React.PointerEvent) => {
    ...
    e.preventDefault();
    panInfo.current = { startX: e.clientX, startY: e.clientY, initialPanX: pan.x, initialPanY: pan.y };
  };
  ...
  useEffect(() => {
    const handlePointerMove = (e: PointerEvent) => {
      if (dragInfo.current) {
        ...
        setPan({ x: initialPanX + (e.clientX - startX), y: initialPanY + (e.clientY - startY) });
      }
    };

    const handlePointerUp = (e: PointerEvent) => {
      if (dragInfo.current) {
        const droppedId = dragInfo.current.nodeId;
        const targetElement = document.elementFromPoint(e.clientX, e.clientY);
        const nodeContainer = targetElement?.closest('.mindmap-node');
        const targetId = nodeContainer?.getAttribute('data-node-id');

        if (targetId && targetId !== droppedId) {
          reparent(droppedId, targetId);
        }
      }
  ...
  const reparent = (nodeId: string, newParentId: string) => {
    const root = rootRef.current;
    let nodeToMove: MindMapNode | null = null;
    const findAndRemove = (current: MindMapNode): MindMapNode => {
      const idx = current.children.findIndex(c => c.id === nodeId);
      if (idx !== -1) { [nodeToMove] = current.children.splice(idx, 1); return { ...current }; }
      return { ...current, children: current.children.map(findAndRemove) };
    };
    const insertIntoParent = (current: MindMapNode): MindMapNode => {
      if (current.id === newParentId && nodeToMove) return { ...current, children: [...current.children, nodeToMove] };
      return { ...current, children: current.children.map(insertIntoParent) };
    };
    const cleaned = findAndRemove(root);
    if (nodeToMove) onChange(insertIntoParent(cleaned));
  };
  ...
  const detachNode = (nodeId: string) => {
    ...
    const cleaned = findAndRemove(root);
    if (nodeToMove) {
      (nodeToMove as any).x = root.x! + 250;
      (nodeToMove as any).y = root.y! + (Math.random() * 200 - 100);
      onChange({ ...cleaned, children: [...cleaned.children, nodeToMove] });
    }
  };
        ...
        >
          <div className="relative flex items-center gap-4">
            ...
            >
              ...
              {/* Hover Dropdown for Subtasks */}
              {hoveredNodeId === n.id && linkedTask && !showingSubtasksId && (
                <div className="absolute top-full left-0 mt-3 w-full bg-white dark:bg-zinc-950 border border-zinc-100 dark:border-zinc-800 rounded-3xl p-4 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] z-[300] animate-in fade-in zoom-in duration-200">
                  <div className="text-[8px] font-black text-zinc-400 uppercase tracking-widest mb-3 px-1 border-b border-zinc-50 dark:border-zinc-900 pb-2">Context Checkitems</div>
                  <div className="space-y-2">
                    {linkedTask.metadata.subTasks?.map((st: any) => (
                      <div key={st.id} className="flex items-center gap-3 px-1">
                        <div className={`w-1.5 h-1.5 rounded-full ${st.checked ? 'bg-cyan-500 shadow-[0_0_8px_rgba(34,211,238,0.5)]' : 'bg-zinc-200 dark:bg-zinc-800'}`} />
                        <span className={`text-[10px] font-bold truncate ${st.checked ? 'text-zinc-400 line-through italic' : 'text-zinc-600 dark:text-zinc-300'}`}>{st.text || 'Draft point...'}</span>
                      </div>
                    ))}
                    {linkedTask.metadata.subTasks?.length === 0 && <div className="text-[9px] text-zinc-300 italic">No checklist items</div>}
                  </div>
                </div>
              )}

              {showingSubtasksId === n.id && linkedTask && (
                <div className="flex flex-col gap-2 mt-2 px-1 animate-in fade-in slide-in-from-top-1">
                  {linkedTask.metadata.subTasks?.map((st: any) => (
                    <div key={st.id} className="flex items-center gap-2">
                      <div className={`w-1 h-1 rounded-full ${st.checked ? 'bg-cyan-500' : 'bg-zinc-300'}`} />
                      <span className={`text-[9px] truncate ${st.checked ? 'text-zinc-400 line-through italic' : 'text-zinc-500'}`}>{st.text}</span>
                    </div>
                  ))}
                </div>
              )}
  ...
  const canvasContent = (
    <div onPointerDown={handleCanvasPointerDown} className="relative select-none overflow-hidden touch-none h-full w-full bg-cyan-500/[0.01] dark:bg-zinc-900/5 cursor-move overscroll-none">
        ...
        <div className="fixed inset-0 z-[400] bg-white dark:bg-zinc-950 animate-in fade-in zoom-in duration-500 overflow-hidden flex flex-col">
          ...
          <div className="h-16 border-t flex items-center justify-between px-12 bg-white dark:bg-zinc-950 shrink-0 border-zinc-100 dark:border-zinc-800 shadow-2xl">
            ...
            <div className="flex items-center gap-10">
               <span className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">• Drag over node for quick checklist</span>
               <span className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">• Click ▲ to toggle sub-mission flow</span>
               <span className="text-[9px] font-black text-cyan-500 uppercase tracking-widest">• Connect nodes to visualize logic</span>
            </div>
          </div>
        </div>
      )}
