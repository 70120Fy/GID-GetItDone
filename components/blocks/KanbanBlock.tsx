...
interface KanbanBlockProps {
  data: KanbanData;
  onChange: (newData: KanbanData) => void;
}

export const KanbanBlock: React.FC<KanbanBlockProps> = ({ data, onChange }) => {
  ...
  return (
    <div className="flex gap-6 overflow-x-auto pb-8 -mx-4 px-4 scrollbar-hide select-none">
      {data.columns.map(column => (
        <div 
          key={column.id} 
          className={`flex-shrink-0 w-72 bg-zinc-50/50 dark:bg-zinc-900/30 rounded-3xl p-5 border transition-all duration-300 flex flex-col gap-4 ${draggedCard && draggedCard.colId !== column.id ? 'border-dashed border-zinc-200 dark:border-zinc-700 bg-zinc-100/30 dark:bg-zinc-800/20' : 'border-zinc-100 dark:border-zinc-800'}`}
        ...
        >
          <div className="flex items-center justify-between px-1 group">
            ...
            <button 
              ...
              className="opacity-0 group-hover:opacity-100 text-zinc-300 dark:text-zinc-700 hover:text-red-400 p-1 transition-all"
            >
          ...
          <div className="flex flex-col gap-3 min-h-[60px]">
            ...
            {column.cards.length === 0 && !draggedCard && (
              <div className="flex-1 flex items-center justify-center border-2 border-dashed border-zinc-50 dark:border-zinc-900 rounded-2xl py-8">
                <span className="text-[10px] font-black text-zinc-200 dark:text-zinc-800 uppercase tracking-widest">Empty</span>
              </div>
            )}
          </div>
      ...
      <button 
        onClick={addColumn}
        className="flex-shrink-0 w-72 h-[300px] border-2 border-dashed border-zinc-50 dark:border-zinc-900 rounded-3xl text-zinc-200 dark:text-zinc-800 hover:border-zinc-200 dark:hover:border-zinc-700 hover:text-zinc-400 dark:hover:text-zinc-500 text-[10px] font-black uppercase tracking-[0.2em] transition-all flex items-center justify-center hover:bg-zinc-50/30 dark:hover:bg-zinc-900/30"
      >
        + Add Column
      </button>
    </div>
